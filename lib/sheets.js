...
async function getTodayRows(todayDate) {
  try {
    ...
    console.log('[sheets.js] Total rows dari Sheets:', allRows.length)
    console.log('[sheets.js] Sample row pertama:', JSON.stringify(allRows[0]))
    console.log('[sheets.js] Sample row kedua:', JSON.stringify(allRows[1]))
    console.log('[sheets.js] Mencari tanggal:', today)

    // Deteksi dinamis jika baris pertama adalah header (kolom pertama bukan angka)
    const hasHeader = allRows.length > 0 && isNaN(Number(allRows[0][0]));
    ...
    console.log('[sheets.js] Semua baris data:', JSON.stringify(dataRows));
