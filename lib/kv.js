...
async function setSchedule(sessions) {
  try {
    await redis.set(KEYS.SCHEDULE, JSON.stringify(sessions));
  } catch (err) {
...
async function getSchedule() {
  try {
    const data = await redis.get(KEYS.SCHEDULE);
...
async function setRadius(meter) {
  try {
    await redis.set(KEYS.RADIUS, Number(meter));
  } catch (err) {
...
async function getRadius() {
  try {
    const r = await redis.get(KEYS.RADIUS);
...
async function markFingerprint(fp) {
  try {
    const key = `${KEYS.FP}:${fp}:${getWIBDate()}`;
    await redis.set(key, '1', { ex: TTL_ONE_DAY });
  } catch (err) {
